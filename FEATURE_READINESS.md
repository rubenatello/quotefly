...
| QuickBooks | Release candidate qualification | CSV/export is supported; owner-observed staging OAuth connect/disconnect passed on a superseded SHA, while the final direct-integration candidate still requires exact-SHA gates and staged sandbox evidence | Production remains disabled and unadvertised. OAuth-only staging proves connection safety without enabling invoice publishing, hosted payments, reconciliation, CDC, or webhook automation; taxable invoices remain blocked | Pass exact-SHA CI and all 88 migrations, complete the authorized final OAuth/replay/disconnect/revocation staging pass, separately authorize accounting mutations, prove signed-webhook/CDC/payment recovery and alert delivery, obtain specialist plus Opera approval, then obtain Intuit production approval and owner go/no-go |
...
QuickBooks-friendly CSV may remain in the controlled beta. Keep direct QuickBooks workflows disabled in production and keep all live-provider claims out of marketing. The explicitly authorized OAuth-only staging phase may enable provider connection while every accounting mutation, worker, hosted-payment, CDC, and webhook-processing capability stays off. The observed sandbox connection is useful provider evidence, but it does not qualify the final SHA or automation. A production pilot still needs the exact-candidate automated gate, all-88-migration backup/forward-fix rehearsal, final OAuth and separately authorized accounting sandbox evidence, payment/refund/CDC recovery evidence, named monitoring and support owners, Sentinel review, independent Opera approval, Intuit production approval, and explicit owner go/no-go.
