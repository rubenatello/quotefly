...
The latest continuation evidence is in the [September 23 staging ledger](quickbooks-staging-evidence-2026-09-23.md). Isolated API/web source 6e25 is deployed with connection-only safeguards. Worker failure/restart and recovery checks passed, but operational email delivery and actual inbox receipt remain unproven. One isolated Neon point-in-time restore and guarded migration rehearsal passed and its temporary resources were removed; this does not establish production-volume recovery or cover later tax-ledger migrations.

The [tax review contract](quickbooks-tax-review-contract.md) and preparatory ledger bind exact invoice inputs and retain uncertain provider-attempt evidence. They have no public route or provider-write adapter and do not enable taxable invoices. Trusted structured addresses, transaction-date policy, provider observations, and real positive-tax Estimate/Invoice parity remain required. Keep these engineering prerequisites separate from advertised product capabilities.
