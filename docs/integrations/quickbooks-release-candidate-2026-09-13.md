...
An owner/admin reviews an internal invoice, maps it to existing QuickBooks Online customers and items, and explicitly publishes it. QuickBooks owns payment processing and settlement. QuoteFly stores encrypted credentials and eligible hosted links, and reconciles provider accounting state into its tenant-scoped ledger. The staging baseline supports non-taxable USD invoices. Customer/item creation, sales-tax publishing, general two-way accounting synchronization, and QuickBooks Desktop integration are not supported by this baseline. The owner subsequently clarified that tax should be handled appropriately for contractor accounts where supported; a non-tax-only public release has not been accepted. A broader tax calculation and reconciliation contract is therefore active implementation scope before tax-capable public claims.
...
1. Complete and verify the contractor-appropriate tax contract requested by the owner. Preserve the explicit non-tax staging boundary while developing provider capability detection, structured addresses, mapped line tax treatment, reviewed calculation provenance, and canonical tax reconciliation. Do not advertise tax sync before that contract passes its provider evidence and final gate.
