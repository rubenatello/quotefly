...
| --- | --- | --- |
| Failed baseline staging/API SHA | `1d987c16ec07804a9404c47648d28e0f5ca3be11` | Historical baseline only: healthy process, but not an acceptable QuickBooks candidate because the owner-observed OAuth callback failed. |
...
| First operational-hardening candidate SHA | `fe4cab7650ca3123cb0b459bf9394ef5a7d79097` on `agent/consumer-launch-readiness` | Contains the 88th migration, durable token-refresh failure state, content-free external signals and machine monitors, bounded shutdown, exact API/worker release checks, provider-attempt health, fail-closed frontend status validation, and rate-limit/proxy hardening. Exact-worktree specialist review subsequently found billing-expiry containment and locked-screen revocation gaps, so this SHA is superseded and must not be promoted. |
| First hardening-candidate CI and web preview | GitHub Actions run `33945706780` was intentionally cancelled after setup/migrations; authoritative `quotefly-web` deployment `dpl_B36Man1AtTdCKQXmoNn8ykAqSMht` passed for `fe4cab7` | The CI cancellation avoided treating a known-defective SHA as releasable. The successful web artifact is historical build evidence only; it was not promoted to staging. |
| Current release-candidate line | Draft [PR #6](https://github.com/rubenatello/quotefly/pull/6) on `agent/qbo-health-rc-20260906`; its current head and corresponding GitHub check are authoritative | This record intentionally does not embed its own final SHA. A newer PR head supersedes every older candidate/run. The staging-bound branch and `main` remain separate. |
| Latest complete local launch gate | Node 22.23.2 executable inputs at `8898237` passed: 406/406 database integration tests and 167 Playwright passes with one optional capture skip; parent exit 0 | `a71667a` changed only three deployment documents. Archive portability remediation is release-tooling-only and requires focused proof plus current-head hosted CI. Do not claim this local command ran on a later SHA. |
| Current exact-SHA Node 22 CI | Follow PR #6's latest-head check. Runs `34066010692` at `8898237` and `34067097892` at `a71667a` passed the full application launch gate but failed overall at image scan evidence validation | Must pass clean install, all 88 migrations, full `verify:launch`, exact image proof, and scan policy on the same current PR head. Successful local scans do not waive hosted failures. |
| Owner-observed staging OAuth lifecycle | Passed on OAuth-only staging SHA `6239f742f04b2815082c0fc7f108727a3e916258`: expected sandbox company connect, status refresh, in-app disconnect/credential cleanup, and empty Intuit integrations screen | Normal lifecycle only. Consumed-callback replay, live revocation/fault recovery, monitoring receipt, and final-candidate revalidation remain pending. |
| Final-candidate staging OAuth proof | Authorized but not executed | Requires ordered exact-head staging deployment and the remaining replay/revocation/monitor checks, with accounting capabilities and worker off. |
| Full accounting sandbox proof | Bounded non-destructive, non-real-money testing authorized but not started | Authorization is not evidence. Execution remains gated on isolated staging, exact-head API/worker parity, signed webhooks, applicable CDC/payment controls, alert delivery, and the owner checklist. Destructive cleanup and real-money/live-book actions are not implied. |
| Production testing / customer pilot | Bounded production testing authorized; no production test or customer pilot has started, and customer pilot/go-live remains unapproved | Requires Intuit production approval/credentials, completed staging and operations evidence, support/accounting/legal acceptance, and explicit final go/no-go before customers or marketing. |
...
- The reviews that preceded `fe4cab7` covered its original scope but did not qualify the exact candidate after the billing lifecycle was examined end to end. Exact-worktree review found that billing-expired tenants could remain eligible for queued provider work and that a billing-locked manager had no safe route to revoke QuickBooks access. Those findings superseded `fe4cab7`, caused CI run `33945706780` to be cancelled deliberately, and reopened specialist approval rather than allowing a known-defective SHA to progress.
- Mutable-worktree review then expanded the remediation beyond the first billing finding: billing eligibility is checked during scheduling and again under a shared tenant lock at claim time; revocation and retention remain active; paused provider queues do not page while raw operator inventory remains visible; the billing-lock screen exposes a manager-only, fail-closed, localized revocation flow; worker/release metadata and the complete status wire contract are validated before rendering; and connection capabilities now match the five-state credential lifecycle. Terminal revocation `ERROR` returns a stable support-required response without provider or database mutation and both UI surfaces remove actions that cannot make progress. Renford and Goldface approved the completed mutable delta, and Sentinel found no Critical, High, or Medium security finding. Every specialist must still confirm the immutable successor SHA and complete evidence; Harbor and Opera have not approved the successor.
...
- The `fe4cab7` pre-BCP `npm run verify:launch` exited zero on 2026-09-04 against a fresh 88-migration PostgreSQL 16 database and live disposable Redis. It passed backend compilation; 127/127 route inventory; production web build, 16-route prerender, 12/12 i18n and 12/12 SEO checks; lint; Prisma validation; 14/14 security checks; the 76-assertion pretest hook including a two-replica Redis quota proof; 244/244 unit tests; 31 integration files and 398/398 database-backed tests in 520.77 seconds; parser 18/18, assistant 101/101, and retrieval 12/12 evaluations; both dependency audits; and Playwright with 143 passed, one intentional capture-regeneration skip, and zero failures in 24.2 minutes. No provider credentials, live tenants, or provider network calls were used. This is now historical regression evidence only because the successor changes executable behavior; the full launch gate must pass again.
- The current successor's focused evidence passes: backend and frontend compilation, frontend lint, Prisma validation, 14/14 security checks, both dependency audits, and the focused setup/operational-health assertions; isolated PostgreSQL 16 worker scheduling, claim, CDC, and monitor coverage passed 4 files and 30/30 tests; the billing-expiry API case passed with connect/setup/mapping blocked and disconnect available; and the full superuser control-plane file passed 7/7 while retaining raw inventory. A first six-case billing-lock browser run passed four and exposed a StrictMode-sensitive test mock plus a real focus-restoration defect. Both were corrected; the clean rerun passed 6/6. Exhaustive status-contract review then found worker/release and credential-lifecycle contradictions. The hardened normalizer and five-state capability table pass 12/12 focused unit tests; terminal-`ERROR` integration proof passes with connect/disconnect both returning support-required `409`, zero provider calls, and unchanged OAuth state, credentials, lifecycle fields, and audit counts; and the combined normal-settings plus billing-lock browser run passes 22/22 with terminal support guidance and no doomed lifecycle actions.
- The first complete successor launch attempt found a real no-connection OAuth race: disconnect returned before invalidating an in-flight authorization state, allowing a callback to connect after the owner saw a successful disconnect. The bounded fix deletes tenant OAuth states under the shared OAuth-init advisory lock before returning an idempotent no-connection disconnect, while terminal `ERROR` remains mutation-free. The exact failing race, its existing-connection counterpart, and the terminal-error invariants passed focused database tests. A later browser attempt exposed three outdated mock payloads that the hardened frontend correctly rejected; the fixtures were made canonical, while the deliberately impossible OAuth-only/provider-disabled state now explicitly proves fail-closed mobile behavior. The focused QuickBooks Settings file then passed 17/17.
- The corrected complete successor `npm run verify:launch` exited zero on 2026-09-05 against isolated PostgreSQL 16 and Redis. It passed backend compilation; 127/127 route inventory; production web build and 16-route prerender; 12/12 i18n and 12/12 SEO; frontend lint; Prisma validation; 14/14 security; the pretest and unit suites; 31 database integration files and 403/403 tests in 483.89 seconds; parser 18/18, assistant 101/101, and retrieval 12/12 evaluations; both dependency audits; and Playwright with 152 passed, one intentional capture-regeneration skip, and zero failures in 23.7 minutes. The two optional live-Redis replica assertions skipped by the default local unit invocation were immediately rerun against the disposable Redis instance and passed within a 4/4 focused rate-limit file set. No provider credentials, live tenants, provider network calls, accounting mutations, or production changes were used. This qualifies the mutable executable worktree; exact-SHA CI and deployment evidence remain authoritative.
...
- PR #5 currently points at superseded SHA `fe4cab7650ca3123cb0b459bf9394ef5a7d79097`. Its intended `Vercel – quotefly-web` artifact passed as deployment `dpl_B36Man1AtTdCKQXmoNn8ykAqSMht`; CI run `33945706780` was cancelled deliberately once specialist findings invalidated promotion. This `quotefly-web` artifact is PR branch-build evidence only, not a staging deployment or promotion source. A second `Vercel – quotefly` context is an obsolete root-project integration and is not release evidence. The successor PR head must pass complete CI and the intended `quotefly-web` context; the obsolete context should be disconnected or explicitly ignored by the owner before branch-protection rules are introduced.
...
- **2026-09-05 historical `6239f742f04b2815082c0fc7f108727a3e916258` checkpoint (not an instruction for a future rollout):** staging discovery first found migration and API services previously deployed by CLI upload with no Git source binding. The historical checkpoint rollout then used GitHub-connected, autodeploy-off Railway services on the then-approved `agent/consumer-launch-readiness` branch, checked Railway-injected Git metadata, and ran **Deploy Latest Commit** in migration-then-API order after the migration ledger passed. The existing QuickBooks worker remained undeployed and off for OAuth-only validation. After API readiness, its staging web deployment used the separate `quotefly-staging` project. This historical fact does not make a `quotefly-web` PR preview staging evidence. Future operators must follow the parameterized exact-SHA procedure in [the QuickBooks sandbox setup runbook](quickbooks-sandbox-setup.md#3-prepare-the-exact-staging-candidate).
...
- [x] Fresh-schema migration and a separate synthetic nonempty 87-to-88 upgrade rehearsal cover all 88 migrations, including the additive token-refresh failure timestamp, idempotent redeploy, unchanged fingerprints, RLS/least-privilege checks, and forward-fix recovery.
...
- [x] Owner authorized bounded staging/production testing and BCP on 2026-09-06. This does not pre-approve customer go-live, destructive cleanup, new paid infrastructure, or real-money actions.
- [ ] Execute and record the ordered exact-SHA staging rollout: migration job, API, API readiness, worker when enabled, then web. OAuth-only keeps the worker off and still deploys web only after API readiness.
...
- [x] Owner authorized bounded non-destructive, non-real-money accounting sandbox testing on 2026-09-06; execution remains unstarted and subject to the listed controls.
- [ ] Complete the staged accounting evidence, including the fabricated non-taxable invoice, hosted-link eligibility, signed webhooks, CDC, timeout, restart and recovery checks. Obtain a separately reviewed scope before destructive void/refund/reversal or any actual funds movement.
...
Provider requirements were rechecked on 2026-09-04 against Intuit's current [production-app publication](https://developer.intuit.com/app/developer/qbo/docs/go-live/publish-app), [redirect URI](https://developer.intuit.com/app/developer/qbo/docs/develop/authentication-and-authorization/set-redirect-uri), [webhook configuration](https://developer.intuit.com/app/developer/qbo/docs/develop/webhooks/configure-webhooks), and [Payments scope](https://developer.intuit.com/app/developer/qbpayments/docs/learn/scopes) documentation. Intuit does not document a generic content-free webhook test event; because notifications are change-triggered, proving genuine Intuit delivery is expected to require a benign disposable sandbox entity mutation. That is an operational inference, not a QuoteFly code requirement. That benign disposable-entity proof is within the recorded bounded sandbox authorization; destructive cleanup, refund/reversal/void, and actual funds movement still require a separately reviewed scope.
...
5. Under the recorded bounded sandbox authorization, advance through the applicable non-destructive portions of `quickbooks-reconciliation`, `quickbooks-cdc`, and `quickbooks-hosted-payments`; for each applicable phase confirm API readiness before starting the same-SHA worker and exposing the phase in web, then complete the applicable owner sandbox checklist. Obtain a separately reviewed scope before destructive void/refund/reversal or actual funds movement.
