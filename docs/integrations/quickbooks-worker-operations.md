...
Workspace billing loss pauses worker-initiated provider reads, token refresh, webhook reconciliation, and CDC before the worker claims provider work. The page scheduler filters ineligible tenants, and each webhook/CDC claim rechecks the current billing row under a shared database lock before issuing the claim. Signed webhook envelopes remain durably queued so they can resume after access is restored. Global alert health masks those deliberately dormant provider queues, while raw superuser tenant inventory remains available for diagnosis. Webhook dead-letter audit, token reauthorization, and connection/orphan revocation alerts remain active across billing states. Billing loss does not pause credential cleanup: an authenticated live owner/admin can still disconnect or immediately retry a pending revocation from an approved QuoteFly origin, and the worker continues pending connection/orphan revocation plus retention passes. A terminal connection `ERROR` means the bounded automatic revocation attempts are exhausted; reconnect and disconnect return a stable support-required response without provider or database mutation, and both normal and billing-locked UI direct the manager to QuoteFly support. All other authenticated QuickBooks mutations remain billing-gated. This lifecycle exception exists only to let a workspace revoke provider access; it does not restore accounting capabilities.
