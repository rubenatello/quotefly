...
| QF-408 | Add automatic customer follow-up schedules and Kody attention retrieval | Implementation complete; exact launch gate and independent reviews pending | New customer intake atomically enrolls the customer in a versioned, tenant-scoped cadence and creates idempotent Activity tasks. Contacted, no-response, and skipped outcomes retain truthful contact history; terminal lifecycle and reassignment transitions are atomic. Settings supports owner/admin editing, member read-only access, English/Spanish parity, mobile controls, stale-version protection, and future-customer-only changes. Kody deterministically returns the earliest visible task per customer with due/urgency/contact-history filters and no phone, email, notes, source keys, vector indexing, or cross-tenant leakage. See [customer follow-up automation](customer-follow-up-automation.md) |
