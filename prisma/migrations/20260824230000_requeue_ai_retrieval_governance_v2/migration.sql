-- The v2 content-governance policy recognizes additional credential formats.
-- Purge every incompatible derived retrieval row before requeueing canonical
-- active sources so legacy excerpts cannot remain readable at rest.
-- Canonical customer, quote, activity, and preset records are not modified.
...
WHERE chunk."documentId" = document."id"
  ...
  AND (
    chunk."chunkerVersion" IS DISTINCT FROM 'nfkc-tiktoken-cl100k-300-overlap36-v1:rag-content-governance-v2'
    OR document."chunkerVersion" IS DISTINCT FROM 'nfkc-tiktoken-cl100k-300-overlap36-v1:rag-content-governance-v2'
  );

DELETE FROM "AiRetrievalDocument"
WHERE "chunkerVersion" IS DISTINCT FROM 'nfkc-tiktoken-cl100k-300-overlap36-v1:rag-content-governance-v2';
...
SELECT
  'idx_governance_v2_' || md5(source."sourceType" || ':' || source."tenantId" || ':' || source."sourceId"),
