...
const REVIEWED_SCHEMA_FIELD_TEXT = {
  ...
  Customer: "archivedAtUtc createdAt deletedAtUtc email followUpStatus followUpUpdatedAtUtc fullName id notes phone phoneDigits tenantId updatedAt",
...
function reviewedShape() {
  return Object.entries(reviewedFields)
    .map(([model, fields]) => ({ model, fields: [...fields].sort() }))
