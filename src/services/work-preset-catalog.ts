...
const STANDARD_WORK_PRESET_CATALOG: Record<ServiceCategory, StandardWorkPresetDefinition[]> = {
  PLUMBING: [
    ...
    standardPreset("fixture_install_package", "Fixture Install Package", "SERVICE", "EACH", 70, 180, "Install or replace a faucet, sink, toilet, or similar plumbing fixture.", 1, ["fixture install", "fixture replacement", "toilet install", "toilet replacement", "faucet install", "faucet replacement", "replace faucet", "sink install", "sink replacement"], { isPrimaryJob: false }),
  ...
  GARDENING: [
    standardPreset("lawn_maintenance_visit", "Lawn Maintenance Visit", "SERVICE", "FLAT", 30, 75, "Flat-rate recurring visit for mowing, edging, blowing, and light cleanup; adjust the price for lot size and visit frequency.", 1, ["lawn maintenance", "mow", "mowing", "yard maintenance", "weekly lawn", "biweekly lawn"], { isPrimaryJob: true }),
    standardPreset("gardening_crew_labor_hour", "Gardening Crew Labor Hour", "LABOR", "HOUR", 42, 95, "Hourly crew labor for variable-scope gardening, pruning, cleanup, or site work that cannot be reliably priced as a flat visit.", 1, ["gardening labor", "crew labor", "hourly gardening", "landscape labor", "yard labor"], { isPrimaryJob: false }),
    ...
    standardPreset("sprinkler_head_replacement", "Sprinkler Head Replacement", "SERVICE", "EACH", 18, 55, "Replace and adjust one damaged spray or rotor head; use quantity for the number of heads.", 1, ["sprinkler head", "rotor head", "spray head", "broken sprinkler"], { isPrimaryJob: false }),
    ...
    standardPreset("shrub_planting", "Shrub Planting", "SERVICE", "EACH", 38, 95, "Install one customer-approved shrub with soil amendment, planting, watering, and cleanup.", 1, ["shrub planting", "plant shrub", "bush planting", "install shrubs"], { isPrimaryJob: false }),
