...
test("a failed customer load cannot masquerade as an empty workspace", async ({ context, page, request }) => {
  ...
  await expect(page.getByText("Customers are temporarily unavailable", { exact: true })).toBeVisible();
  ...
  await expect(page.getByText("Add your first customer", { exact: true })).toBeVisible();
