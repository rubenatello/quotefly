...
test("individual save and line insertion preserve other drafts through Save Quote and reload", async ({
  ...
  await firstRow.getByLabel("Existing line 1 description").fill("Saved first-row description");
  await firstRow.getByLabel("Existing line 1 quantity").fill("2");
  await firstRow.getByLabel("Existing line 1 price").fill("1275");
  await secondRow.getByLabel("Existing line 2 description").fill("Unsaved second-row description");
  await secondRow.getByLabel("Existing line 2 quantity").fill("3");
  await secondRow.getByLabel("Existing line 2 cost").fill("225");
  await secondRow.getByLabel("Existing line 2 price").fill("375");

  await firstRow.getByRole("button", { name: "Save line" }).click();
  await expect(firstRow.getByText("Unsaved")).toHaveCount(0);
  await expect(secondRow.getByLabel("Existing line 2 description")).toHaveValue("Unsaved second-row description");
  await expect(secondRow.getByLabel("Existing line 2 quantity")).toHaveValue("3");
  await expect(secondRow.getByLabel("Existing line 2 cost")).toHaveValue("225");
  await expect(secondRow.getByLabel("Existing line 2 price")).toHaveValue("375");
  ...
  await expect(page.getByLabel("Quote status")).toHaveValue("READY_FOR_REVIEW");
  await expect(page.getByLabel("Tax")).toHaveValue("245");
  await expect(page.getByTestId("existing-quote-line-row-1").getByLabel("Existing line 1 description")).toHaveValue(
    "Saved first-row description",
  );
  await expect(page.getByTestId("existing-quote-line-row-2").getByLabel("Existing line 2 description")).toHaveValue(
    "Unsaved second-row description",
  );
  await expect(page.getByTestId("existing-quote-line-row-2").getByLabel("Existing line 2 quantity")).toHaveValue("3");
  await expect(page.getByTestId("existing-quote-line-row-3").getByLabel("Existing line 3 title")).toHaveValue(
...
test("failed quote metadata save stops line writes and retains all quote-sheet drafts", async ({
  ...
  const firstRow = page.getByTestId("existing-quote-line-row-1");
  await firstRow.getByLabel("Existing line 1 description").fill("Line draft must not be sent after metadata failure");
  const newRow = page.getByTestId("new-quote-line-row");
  ...
  await expect(firstRow.getByLabel("Existing line 1 description")).toHaveValue(
